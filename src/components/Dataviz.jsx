		...
		function filteredDataset(dataDay, dataEve) {
			...
			nlData().then((data) => {
				...
				const gemeentes = g
					...
					.selectAll('path')
					.data(
						topojson.feature(data, data.objects.gemeente_2020)
							.features
					)
					.join('path')
					.on('click', clicked)
			...
			function dots(dotData) {
				...
				function reassignDots(data, color, strokeColor) {
					const dots = g.selectAll('circle').data(data); // Assign new data to the dots
				...
				function updatingMapAvailable(data) {
					const availableValues = data.filter(function (d) {
						return (
							Number(d.status.available) > 0 &&
							Number(d.status.charging) >= 0
						);
					});
					console.log('Beschikbaar:', availableValues);
					reassignDots(
						availableValues,
						'rgba(34, 219, 13, 0.349)',
						'rgb(34, 219, 13)'
					);
				}
				...
				function updatingMapBusy(data) {
					const chargingValues = data.filter(function (d) {
						return (
							Number(d.status.charging) > 0 &&
							Number(d.status.available >= 0)
						);
